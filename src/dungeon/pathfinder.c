...
graph_t* path_construct(int invert) {
...
void path_pathfind(graph_t* g, point_t* start, point_t* end) {
    dijkstraAPI.dijkstra(g, start, end, path_point_to_index);
    place_path(g, end);
}
...
static void place_path(graph_t* g, point_t* b) {
    ...
    vertex_t* v;
    point_t p;
    v = g->vertices[path_point_to_index(b)];
    if(v->dist == INT_MAX) {
